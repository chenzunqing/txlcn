...
public class TMAutoCluster implements TxLcnInitializer {
    ...
    public void init() throws Exception {
        String advertiseIp = txManagerConfig.getAdvertiseIp();

        // 1. 通知 TC 建立连接
        List<TMProperties> tmList = fastStorage.findTMProperties().stream()
                .filter(tmProperties ->
                        !tmProperties.getHost().equals(advertiseIp) || !tmProperties.getTransactionPort().equals(txManagerConfig.getPort()))
                .collect(Collectors.toList());
        for (TMProperties properties : tmList) {
            NotifyConnectParams notifyConnectParams = new NotifyConnectParams();
            notifyConnectParams.setHost(advertiseIp);
    ...
    public void destroy() throws Exception {
        fastStorage.removeTMProperties(txManagerConfig.getAdvertiseIp(), txManagerConfig.getPort());
    }
