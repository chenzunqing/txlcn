...
public class SocketManager {
    ...
    @Getter
    private final Map<String, AppInfo> appNames;
    ...
    public List<String> remoteKeys(String modId) {
        ...
        log.debug("AppNames: {}", appNames);
        for (Channel channel : channels) {
            AppInfo appInfo = appNames.get(channel.remoteAddress().toString());
            if (appInfo == null) {
                log.warn("remote peer({})'s modId is missing.", channel.remoteAddress());
                channel.close();
                continue;
            }
            if (modId.equals(appInfo.getLabelName())) {
                allKeys.add(channel.remoteAddress().toString());
            }
        }
        return allKeys;
